...
New UI scenarios explicitly select `battery.consumptionModel = active_time`, with 70 percentage points per 360 active minutes. The rate is a supplier assumption relayed by the user; driving/handling scope is user-confirmed. Driving to charge is included; idle, blocked unloading and charger queues are excluded. An absolute active-millisecond ledger avoids event-splitting rounding drift. Charge gain follows the scenario's existing linear rate. `RUN_ENDED` records partially completed work/charge at the horizon; negative energy fails. The UI projects battery from the latest saved state and elapsed time using the same pure model, without generating a second simulation. Missing model or `per_task` preserves the legacy completion-time deduction. Threshold checks remain at task completion; mid-task interruption and energy reservation are not implemented.
