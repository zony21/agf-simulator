...
CADプレビューを読み込んだ後、「仮線を引く」で始点と終点を指定すると、直線／横→縦／縦→横の**暫定幾何線**を生成できる。自動生成は通路・障害物・シャッターを判別する経路探索ではない。利用者は「線を修正」で点をドラッグし、区間をクリックして経由点を挿入、経由点・経路を削除し、拡大・元に戻す・やり直しを使える。スマートフォンではタッチ操作に対応する。共有点を動かすと他経路にも影響するため、確認ダイアログを表示する。

搬送01～05・充電、空走／積載／充電区間は経路ごとに分けて保存する。ファイルのハッシュとviewBoxに紐づいたJSONを端末内で入出力する。すべて `draft-only`・`routable=false`・`physicalEtaAllowed=false` のままであり、座標や経路下書きを公開Gitに配置しない。実ルートとしての承認・走行時間計算は**別工程**で、線を引いただけでは実機の移動を再現できない。

### 初期参考ラインの読込と利用者による修正

非公開のCAD SVGと、その同じファイルのSHA-256/viewBoxに結び付けた**初期参考ラインJSON**を利用できる。出力は `guide/guide` の未割当・`draft-only` に限定し、搬送01〜05や充電に誤割当しない。参考線は現場経路を自動識別したものではなく、図面上の編集開始位置を示す仮線であり、車体寸法・障害物・シャッター条件・荷役姿勢・通行許可は未検証。利用者が図上で直した後、対象線を選択し、明示的に搬送種別と区間を割り当てられるが、それでも実走行経路へは昇格しない。

新たな初期案の作成は `tools/build-private-route-seed.mjs` を使用する。入力は実図由来の非公開SVGと、監修対象の正規化座標を含む**非公開**のJSON設定。共通接続点は同じ `{id,u,v}` を複数のガイドで参照可能で、点の移動は関連経路にも反映される。出力は必ずprivate/等に置き、GitHubへ元図・設定・派生JSON・スクリーンショットを載せない。

~~~bash
node tools/build-private-route-seed.mjs \
  --svg private/preview.svg \
  --config private/guide-seed.json \
  --out private/initial-guides.json
~~~

テスト：`node --test tests/private-route-seed.test.mjs`。入力点を自動で「通行可能」と扱わない。起終点・方向・停止／旋回・ゲート条件が利用者確認済みになった経路だけ、別の承認／走行トポロジー化工程で扱う。
